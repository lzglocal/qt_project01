...
void* AES::inv_cipher(void* input, int length)
{
    unsigned char* in = (unsigned char*) input;
    int i;
    for(i=0; i<length; i+=16)
...
void AES::key_expansion(unsigned char* key, unsigned char w[][4][4])
{
    int i,j,r,c;
...
void AES::add_round_key(unsigned char state[][4], unsigned char k[][4])
{
    int r,c;
    for(c=0; c<4; c++)
    {
...
void AES::inv_sub_bytes(unsigned char state[][4])
{
    int r,c;
    for(r=0; r<4; r++)
    {
...
void AES::inv_shift_rows(unsigned char state[][4])
{
    unsigned char t[4];
    int r,c;
    for(r=1; r<4; r++)
...
void AES::inv_mix_columns(unsigned char state[][4])
{
    unsigned char t[4];
    int r,c;
    for(c=0; c< 4; c++)
...
//构造函数
Aes::Aes(int key_size,unsigned char* key_bytes)
{
    SetNbNkNr(key_size); //设置密钥块数，轮数
    memcpy(key,key_bytes,key_size);                //字符串拷贝函数，把keyBytes的keysize个字符复制到key中
...
void Aes::KeyExpansion(){
    ...
    for(int row=Nk;row<4*(Nr+1);row++)
    {
        ...
        if(row%Nk==0) //逢nk时，对当前列的前一列作特殊处理
        {
            ...
            temp[0] = (unsigned char)( (int)temp[0] ^ (int) aes_rcon[4*(row/Nk)+0] );
            temp[1] = (unsigned char)( (int)temp[1] ^ (int) aes_rcon[4*(row/Nk)+1] );
            temp[2] = (unsigned char)( (int)temp[2] ^ (int) aes_rcon[4*(row/Nk)+2] );
            temp[3] = (unsigned char)( (int)temp[3] ^ (int) aes_rcon[4*(row/Nk)+3] );
        }
...
unsigned char* Aes::SubWord(unsigned char* word)
{
    ...
    for(int j=0;j<4;j++)
    {
        temp[j] = aes_sbox[16*(word[j] >> 4)+(word[j] & 0x0f)]; //实际上也可以写成AesSbox[[j]];因为两者相等
    }
    return temp;
...
void Aes::InvSubBytes()
{
    ...
    for(j=0;j<4;j++)
    {
        for(i=0;i<4;i++)
        {
            State[i][j]=aes_rsbox[State[i][j]]; //因为 16*(State[i][j]>>4)+State[i][j]&0x0f=State[i][j]
        }
    }

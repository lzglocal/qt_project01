...
public:
    ...
    //弹出框选项参数1标题，参数2显示信息 参数类型框3：0是信息框 1错误信息框 2提醒框 3自定义头像和ico框 4询问框yes_or_no
    //参数4:关闭窗口毫秒
    static int showMessageBox(const QString &title, const QString &info, int type = 0, int closeSec = 0, bool exec = false);
    //消息框YES
    static void showMessageBoxInfo(const QString &title,const QString &info, int closeSec = 0, bool exec = false);
   //提醒框yes
    static void showMessageBoxWarn(const QString &title,const QString &info, int closeSec = 0, bool exec = false);
    //错误框YES
    static void showMessageBoxError(const QString &title,const QString &info, int closeSec = 0, bool exec = false);
    //自定义提示框YES
    static void showMessageBoxCustomize(const QString &title,const QString &info, int closeSec = 0, bool exec = false);
    //询问框Yes_No
    static int showMessageBoxQuestion(const QString &title, const QString &info);
    ...
    static QString getOpenFileName(const QString &filter = QString(),
                                   ...
                                   bool native = false, int width = 550, int height = 400);
    //保存文件对话框
    static QString getSaveFileName(const QString &filter = QString(),
                                   const QString &dirName = QString(),
                                   const QString &fileName = QString(),
                                   bool native = false, int width = 450, int height = 350);
    //选择目录对话框
    static QString getExistingDirectory(const QString &dirName = QString(),
                                        bool native = false, int width = 700, int height = 600);
